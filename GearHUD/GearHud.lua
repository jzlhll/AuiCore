...
local function InitAddonPanel()
    ...
    fs2:SetJustifyV("TOP")
    fs2:SetText("在屏幕中显示红色齿轮警示，防止在战斗中忘记自身血量过低。")

    local fs3 = panel:CreateFontString(nil, "OVERLAY", "TextStatusBarText")
    fs3:SetPoint("TOPLEFT", 10, -100)
    fs3:SetPoint("BOTTOMRIGHT", fs2, "TOPRIGHT", 0, -80)
    fs3:SetJustifyH("LEFT")
    fs3:SetJustifyV("TOP")
    fs3:SetText("点击解锁，拖动位置，或者拖动右下边角进行缩放。再次点击，锁定位置完成。")
    local button = CreateFrame("Button", nil, panel, "UIPanelButtonTemplate")
    button:SetText("锁定/解锁位置")
    button:SetWidth(128)
    button:SetPoint("TOPLEFT", 10, -135)
    ...
    button2:SetText("效果测试")
    button2:SetWidth(128)
    button2:SetPoint("TOPLEFT", 200, -155)
